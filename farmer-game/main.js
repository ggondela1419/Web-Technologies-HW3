...
class Crop extends Entity {
    ...
    static TYPES = [
        ...
        { name : "golden apple", points: 5, color: "#ffd700" },
    ];

   // returns a random crop type name
    static randomType() {
...
class Input {
    constructor(game) {
        ...
        //This bind is needed because event listeners typically assign "this" to the window
        //object. Using .bind(this) ensures that the handler retains the correct context of the Input instance.
        this._onKeyUp = this.onKeyUp.bind(this);   // bind #2
        //Same logic as the previous bind. If we don't bind, "this" won't point to the Input class and
        //this.keys.delete(e.key) will throw an error.
...
class Game {
    ...
    spawnCrop() {
        ...
        const cropType = Crop.randomType();
    ...
    update(dt) {
        ...
        //Difficulty curve: decrease spawnEvery as time passeses
        const progress = 1 - this.timeLeft / GAME_LEN; // 0 at start, 1 at end
        const minSpawn = 0.4; // minimum spawn interval
        const maxSpawn = 0.8; // maximum spawn interval
        this.spawnEvery = maxSpawn - (maxSpawn - minSpawn) * progress;
        ...
        //using an arrow here means it keeps the same "this" as the Game class,
        //so I do not lose access to the game's properties
        //all the callbacks are arrows so "this" is safe
        if (collected.length) {
            collected.forEach(crop => {
                this.score += crop.points;
                crop.dead = true;
            });                             // arrow #2
            //Using an arrow function here prevents a new "this" binding from being created,
            //which would cause the context to be lost inside the callback.
            if (this.ui.score) this.ui.score.textContent = String(this.score);
        ...
        //Arrow functions provide lexical scoping for "this", which makes them ideal
        //for array processing methods like filter and map.
        this.crops.forEach(c => c.update(dt, this));                         // arrow #4
        //The arrow function here ensures that the correct "this" context" is applied
        //when iterating through the crops and updating each one's state.
    ...
    render() {
        ...
        //Using an arrow function here makes it easy to iterate through crops while directly
        //calling their draw method, without worrying about the "this" context.
        this.obstacles.forEach(o => o.draw(ctx));                             // arrow #6
        //This arrow function goes through obstacles and calls each one's draw method,
        //keeping the "this" context intact and not changing the scope. 
